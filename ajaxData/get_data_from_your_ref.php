...
$query_yourref = "select SUM(ISNULL(aantal,0)) as qty from gbkmut where RTRIM(LTRIM(docnumber))=? and RTRIM(LTRIM(artcode))=? and transtype='N' and reknr IN (117111,117114) and transsubtype='B'";
...
if(is_array($arr_return)&&sizeof($arr_return)>0){
	if($arr_return[0]['qty']>0){
		echo json_encode(array(true,$arr_return[0]['qty'],$_POST['company_code'],$_POST['your_ref'],$_POST['itemcode']));
	}else{
