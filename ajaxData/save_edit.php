...
$params_save = array($_POST['company_source'],$_POST['div_source'],$_POST['company_destination'],$_POST['div_destination'],$_POST['reason'],$_POST['reason_detail'],$_POST['note'],$today,1,$_POST['save_status']);
...
if(is_array($arr_query)&&sizeof($arr_query)>0){
	...
	while($i<sizeof($_POST['array_item_insert'])){
		...
		if($arr_item[10]==0) // delete create or update
		{
	...
	if($_POST['type_status']=="edit"){
      ...
      ,[status]
      ,[status_note]) OUTPUT Inserted.atid as id values (?,?,?,?,?,?,?)";
		$params_flow = array($_POST['userid'],$_POST['username'],$arr_query[0]['id'],$today,"re-open",$_POST['save_status'],NULL);
		$arr_query_flow=$local_db->query_data($q_flow,$params_flow);
	}
