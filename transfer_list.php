...
<html>
	        ...
	        <div class="container">
		        ...
		        <div id="navbar" class="navbar-collapse collapse">
		          <ul class="nav navbar-nav">
		          	...
		          	<li role="menu"><a href="transfer_list.php" class="active">รายการรอตัดโอน</a></li>
		            <li class="dropdown">
    ...
    <div class="content">
    	<div class="inner-content">
    		<table width="100%" id="table_transfer_list" class="table-main">
    			<thead>
    				<tr>
    					<td>id</td>
    					<td width="13%">ตัดโอน</td>
    					<td width="11%">บริษัท-ฝ่ายร้องขอ</td>
    					<td width="11%">บริษัท-ฝ่ายเจ้าของ</td>
    			...
    			<tbody>
    				<?php
   					$q_trans = "select rt.*,c.des as cause_des,s.name as status_des 
from reserve_transaction rt 
left join cause c on rt.cause_id=c.atid 
left join status s on rt.status=s.atid 
where rt.destination_cmp='".$company_allowed."' and rt.status=3 and rt.row_active=1";
    				...
    				while($i<sizeof($arr_trans)){
    					...
    					<tr>
    						...
    						<td nowrap><a onclick="cutting_stock(<?=$arr_trans[$i]["atid"];?>,'<?=basename(__FILE__, '.php');?>');"><img src="img/stock.png" width="30" height="30"  class="edit_data" class="form_acknowledge">&nbsp;<?=$arr_trans[$i]['reserve_no'];?></a></td>
...
var table_transfer_list=$('#table_transfer_list').DataTable({"dom":'<f<t>>',"columnDefs": [{"targets": [0],"visible": false,"searchable": false}],"bPaginate": true,"bPagelength":10,"bSort":true});
